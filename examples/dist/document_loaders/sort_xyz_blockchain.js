...
export const run = async () => {
    ...
    */
    ...
    const nftPrompt = "Describe the character with the attributes from the following json document in a 4 sentence story. ";
    const nftResponse = await model.call(nftPrompt + JSON.stringify(nftMetadataDocs[0], null, 2));
    ...
    */
    ...
    const latestPrompt = "Describe the following json documents in only 4 sentences per document. Include as much detail as possible. ";
    const latestResponse = await model.call(latestPrompt + JSON.stringify(latestTransactionsDocs[0], null, 2));
    console.log(`\n\nuser > ${nftPrompt}`);
    ...
    */
    ...
    const sqlPrompt = "Describe the character with the attributes from the following json document in an ad for a new coffee shop. ";
    const sqlResponse = await model.call(sqlPrompt + JSON.stringify(sqlDocs[0], null, 2));
    console.log(`\n\nuser > ${sqlPrompt}`);
